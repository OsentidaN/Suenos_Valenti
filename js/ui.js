    ...
    sesiones.forEach(sesion => {
        ...
        card.innerHTML = `
            <div class="contenedor-imagen-tarjeta">
                <img src="${imagenSrc}" alt="${sesion.nombre}" class="imagen-tarjeta">
                <div class="insignia-precio-tarjeta">${sesion.precio}€</div>
            </div>
            
            <div class="contenido-tarjeta">
                <h3>${sesion.nombre}</h3>
                <p class="descripcion-tarjeta">${sesion.descripcion}</p>
...
export function renderizarCarrito(carrito, callbackEliminar, callbackCantidad) {
    ...
    carrito.forEach(item => {
        ...
        itemEl.innerHTML = `
            ...
            <p class="meta-item-carrito">${item.duracion}</p>
            ...
            <div class="controles-item-carrito">
                <div class="controles-cantidad">
                    ...
                    <span class="cantidad-display">${item.cantidad || 1}</span>
                    <button class="boton-cantidad btn-mas" data-id="${item.id}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
                    </button>
                </div>
                <span class="precio-item">${(item.precio * (item.cantidad || 1)).toFixed(2)}€</span>
            </div>
        `;
